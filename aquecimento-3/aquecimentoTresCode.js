...
const Plotly_1 = require("../libs/Plotly");
//
// HTML Elements and Constants
//
const TESTER = document.getElementById('tester');
Plotly_1.Plotly.newPlot(TESTER, [{
        x: [1, 2, 3, 4, 5],
        y: [1, 2, 4, 8, 16]
    }], {
    margin: { t: 0 }
...
function waveSum(waves, time_0, time_f, step) {
    const values = [];
    if (step <= 0) {
        return undefined;
    }
...
function diceCenter(length, diceDensity, dotDensity, dotRadius) {
    const b = (4 / 3) * Math.PI;
